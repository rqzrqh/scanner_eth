...
type TxInternal struct {
	...
	Height       uint64 `gorm:"index:idx_height;type:bigint unsigned;comment:block_height"`
	TxHash       string `gorm:"index:idx_txhash;type:varchar(255);comment:tx_hash"`
	Index        int    `gorm:"type:int;comment:index"`
	From         string `gorm:"index:idx_from;type:varchar(255);comment:from_addr"`
	To           string `gorm:"index:idx_to;type:varchar(255);comment:to_addr"`
