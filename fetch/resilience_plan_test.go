...
func TestPlanP2DBIntermittentFailureThenRecovery(t *testing.T) {
	...
	if fm.pendingPayloadStore.GetBlockHeader("b") == nil || fm.pendingPayloadStore.GetBlockBody("b") == nil {
...
func TestPlanP3HeaderFetchTimeoutAndDisorderConverges(t *testing.T) {
	...
	fm.blockFetcher = &mockBlockFetcher{
		...
		fetchByHeightFn: func(_ context.Context, nodeOp *NodeOperator, taskID int, height uint64) *BlockHeaderJson {
			if height != 11 {
				return nil
			}
			height11Calls++
