...
func TestSyncNodeDataByHashEarlyReturnAndFailures(t *testing.T) {
	...
	if !fm.syncNodeDataByHash(context.Background(), "") {
		t.Fatal("empty hash should return true (noop)")
	}
	if !fm.syncNodeDataByHash(context.Background(), "0xnotfound") {
	...
	if fm.syncNodeDataByHash(context.Background(), "0x10") {
...
func TestSyncNodeDataByHashDecodeFallbackAndFullBlockFail(t *testing.T) {
	...
	if fm.syncNodeDataByHash(context.Background(), "0x12") {
...
func TestScanFlowEdgeBranches(t *testing.T) {
	...
	if ok, msg := fm.syncBodyTarget(context.Background(), "   "); ok || msg == "" {
	...
	fm.processBranchNode(context.Background(), nil) // no panic
}
...
func TestSyncBodyTargetStopsWhenContextCancelled(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	ok, msg := fm.syncBodyTarget(ctx, "h1")
	if ok || msg == "" {
