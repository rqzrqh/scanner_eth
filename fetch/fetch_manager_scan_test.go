...
func newTestFetchManager(t *testing.T, irreversible int) *FetchManager {
	...
	fm.nodeManager = NewNodeManager([]*ethclient.Client{nil})
...
func makeEventBlockData(height uint64, hash string, parent string) *EventBlockData {
	return &EventBlockData{
		...
		ProtocolFullBlock: &protocol.FullBlock{
			Block: &protocol.Block{Height: height, Hash: hash, ParentHash: parent},
		},
	}
}
...
type mockDbOperator struct {
	loadFn  func() ([]model.Block, error)
	storeFn func(blockData *EventBlockData) error
}

func (m *mockDbOperator) LoadBlockWindowFromDB() ([]model.Block, error) {
...
func (m *mockDbOperator) StoreBlockData(blockData *EventBlockData) error {
	if m != nil && m.storeFn != nil {
		return m.storeFn(blockData)
	}
	return nil
}
...
func (m *mockBlockFetcher) FetchBlockHeaderByHeight(nodeID int, taskID int, client *ethclient.Client, height uint64) *BlockHeaderJson {
	if m != nil && m.fetchByHeightFn != nil {
		return m.fetchByHeightFn(nodeID, taskID, client, height)
	}
	return nil
}

func (m *mockBlockFetcher) FetchBlockHeaderByHash(nodeID int, taskID int, client *ethclient.Client, hash string) *BlockHeaderJson {
	if m != nil && m.fetchByHashFn != nil {
		return m.fetchByHashFn(nodeID, taskID, client, hash)
	}
	return nil
}

func (m *mockBlockFetcher) FetchFullBlock(nodeID int, taskID int, client *ethclient.Client, header *BlockHeaderJson) *data.FullBlock {
	if m != nil && m.fetchFullFn != nil {
		return m.fetchFullFn(nodeID, taskID, client, header)
	}
	return nil
}
...
func TestScanEventsRule1BootstrapOnEmptyTree(t *testing.T) {
	...
	fm.blockFetcher = &mockBlockFetcher{fetchByHeightFn: func(nodeID int, taskID int, client *ethclient.Client, height uint64) *BlockHeaderJson {
		if height != 100 {
	...
	fm.dbOperator = &mockDbOperator{
		storeFn: func(blockData *EventBlockData) error {
			if blockData != nil && blockData.ProtocolFullBlock != nil && blockData.ProtocolFullBlock.Block != nil {
				writeOrderMu.Lock()
				writeOrder = append(writeOrder, blockData.ProtocolFullBlock.Block.Hash)
				writeOrderMu.Unlock()
			}
			return nil
		},
...
func TestTaskPoolAsyncDeduplicated(t *testing.T) {
	...
	var handled int32
	fm.blockFetcher = &mockBlockFetcher{fetchFullFn: func(nodeID int, taskID int, client *ethclient.Client, header *BlockHeaderJson) *data.FullBlock {
...
func TestRestoreBlockTreeLoadsWindowAndCompleteState(t *testing.T) {
	...
	fm := newTestFetchManager(t, 2)
	fm.db = db
	fm.dbOperator = newFetchManagerDbOperator(fm.db, fm.irreversibleBlocks)

	windowBlocks, err := fm.dbOperator.LoadBlockWindowFromDB()
	if err != nil {
...
func TestRestoreBlockTreeFailsWhenParentMissingAndTreeHasNoRoot(t *testing.T) {
	db := newTestDB(t)
	blocks := []model.Block{
	...
	loaded, err := fm.restoreBlockTree(windowBlocks)
	if err == nil {
		t.Fatalf("expected load to fail when first block parent missing and tree has no root")
	}
	if loaded != 0 {
		t.Fatalf("expected loaded=0 on failure, got=%v", loaded)
	}
}
