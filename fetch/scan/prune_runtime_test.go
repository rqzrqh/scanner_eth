...
func TestComplexOrphanCascadeInsertionTracksTreeAndStoredRange(t *testing.T) {
	...
	env.blockTree.Insert(1, "a", "", 1, nil)
	env.blockTree.Insert(5, "e", "d", 1, nil)
	env.blockTree.Insert(3, "c", "b", 1, nil)
	env.blockTree.Insert(7, "g", "f", 1, nil)
	env.blockTree.Insert(6, "f", "e", 1, nil)
	env.blockTree.Insert(2, "b", "a", 1, nil)
	env.blockTree.Insert(4, "d", "c", 1, nil)
	env.blockTree.Insert(4, "x", "c", 1, nil)
	env.blockTree.Insert(5, "y", "x", 1, nil)
...
func TestInvariantPruneDeletesPendingAndStored(t *testing.T) {
	...
	for h := uint64(1); h <= 6; h++ {
		...
		env.pending.SetNodeBlockHeader(hash, makeTestHeader(h, hash, parent))
		env.pending.SetNodeBlockBody(hash, makeTestBody(true))
		parent = hash
	}
	...
	for _, hash := range []string{"h1", "h2", "h3"} {
		...
		if env.pending.GetNodeBlockHeader(hash) != nil {
			t.Fatalf("expected %s header removed from pending blocks after prune", hash)
		}
		if env.pending.GetNodeBlockBody(hash) != nil {
...
func TestPruneComplexForkRemovesPrunedBranchesAndStoredState(t *testing.T) {
	...
	env.blockTree.Insert(4, "d", "c", 1, nil)
	env.blockTree.Insert(5, "e", "d", 1, nil)
	env.blockTree.Insert(3, "x", "b", 1, nil)
	env.blockTree.Insert(4, "y", "x", 1, nil)
...
func TestPruneFrequentPruneAndStoreAlternationConsistency(t *testing.T) {
	...
	for h := uint64(1); h <= 12; h++ {
		...
		env.blockTree.Insert(h, hash, parent, 1, nil)
		env.pending.SetNodeBlockHeader(hash, makeTestHeader(h, hash, parent))
		env.pending.SetNodeBlockBody(hash, makeTestEventBlockData(h, hash, parent))
		parent = hash
	}
...
func TestPruneNoopWhenStoredSpanWithinKeepEvenWithStaleStoredHashes(t *testing.T) {
	...
	env.blockTree.Insert(10, "a", "", 1, nil)
	env.blockTree.Insert(11, "b", "a", 1, nil)
	env.blockTree.Insert(12, "c", "b", 1, nil)
	env.stored.MarkStored("a")
...
func TestPruneOrphanCapacityPressureConverges(t *testing.T) {
	...
	for h := uint64(1); h <= 30; h++ {
		hash := fmt.Sprintf("h%d", h)
		bt.Insert(h, hash, parent, 1, nil)
		parent = hash
	}
	for h := uint64(2); h <= 21; h++ {
		bt.Insert(h, fmt.Sprintf("oa%d", h), fmt.Sprintf("ma%d", h), 1, nil)
	}
	for h := uint64(40); h <= 99; h++ {
		bt.Insert(h, fmt.Sprintf("ob%d", h), fmt.Sprintf("pb%d", h), 1, nil)
	}
	...
	for h := uint64(40); h <= 69; h++ {
		bt.Insert(h-1, fmt.Sprintf("pb%d", h), "h30", 1, nil)
		bt.Prune(1)
	}
