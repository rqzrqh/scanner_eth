...
func (fm *FetchManager) runScanCoordinator(ctx context.Context) bool {
	...
	if len(bodyTargets) > 0 {
		fm.runScanStageAsync(ctx, scanStageBodyDone, strings.Join(bodyTargets, ","), fm.syncBodyTarget, func(event scanStageEvent) {
			fm.logScanStageEvent(event)
			if event.success && ctx.Err() == nil {
...
func (fm *FetchManager) handleTaskPoolTask(task *syncTask, stopCh <-chan struct{}) bool {
	...
	if task.kind == syncTaskKindBody || (task.kind == 0 && task.hash != "") {
		...
		success := fm.syncNodeDataByHash(ctx, task.hash)
		close(done)
		select {
		case <-stopCh:
			return false
...
// runScanStageAsync runs fn on a detached goroutine. fn must respect ctx cancellation so work
// does not continue after stopScanLoop (e.g. on lost leader) when deleteRuntimeState nils blockTree.
func (fm *FetchManager) runScanStageAsync(ctx context.Context, stage scanStage, target string, fn func(context.Context, string) (bool, string), after func(scanStageEvent)) bool {
	go func() {
		startedAt := time.Now()
		if ctx.Err() != nil {
			if after != nil {
				after(scanStageEvent{
					stage:    stage,
					target:   target,
					success:  false,
					duration: time.Since(startedAt),
					errMsg:   "scan context cancelled",
				})
			}
			return
		}
...
func (fm *FetchManager) syncBodyTarget(ctx context.Context, target string) (bool, string) {
	if strings.TrimSpace(target) == "" {
		return false, "invalid body target"
	}
	if fm == nil || fm.blockTree == nil {
		return false, "fetch manager or blocktree not available"
	}
	...
	for _, raw := range hashes {
		...
		fm.storeNodeBodyData(ctx, node, nodeData, state)
	}
	return true, ""
}
...
func (fm *FetchManager) ensureBootstrapHeader() bool {
	...
	success := false
	for _, nodeOp := range fm.nodeManager.NodeOperators() {
		if nodeOp == nil {
			continue
		}
		header := fm.blockFetcher.FetchBlockHeaderByHeight(context.Background(), nodeOp, 0, height)
		if header == nil {
			continue
		}
		fm.insertHeader(header)
		logrus.Infof("bootstrap blocktree root by startHeight success. height:%v hash:%v node:%v", height, normalizeHash(header.Hash), nodeOp.ID())
		success = true
	}
	if !success {
...
func (fm *FetchManager) processBranchesLowToHigh(ctx context.Context) {
	...
	for _, branch := range branches {
		for i := len(branch.Nodes) - 1; i >= 0; i-- {
			fm.processBranchNode(ctx, branch.Nodes[i])
		}
	}
}

func (fm *FetchManager) processBranchNode(ctx context.Context, node *blocktree.LinkedNode) {
	if node == nil {
		return
	}
...
func (fm *FetchManager) storeNodeBodyData(ctx context.Context, node *blocktree.LinkedNode, nodeData *EventBlockData, state branchProcessState) {
	if ctx.Err() != nil {
		return
	}
	if !fm.hasStorableNodeData(nodeData) {
	...
	if err := fm.dbOperator.StoreBlockData(ctx, blockData); err != nil {
...
func (fm *FetchManager) hasStorableNodeData(nodeData *EventBlockData) bool {
	...
	return nodeData.StorageFullBlock != nil
}
