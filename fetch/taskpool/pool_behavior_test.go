...
func TestTaskPoolHeaderHashNormalizationTrimsWhitespace(t *testing.T) {
	tp := Pool{
		Tracked:     make(map[string]struct{}),
		QueueHigh:   make(chan *SyncTask, 8),
		QueueNormal: make(chan *SyncTask, 8),
	...
	if !tp.EnqueueHeaderHashTask(" 0xAb ") {
		t.Fatal("expected header hash task to enqueue")
	}
	if !tp.IsHeaderHashSyncing("0XaB") {
		t.Fatal("expected header hash syncing lookup to use normalized hash")
	}
}
