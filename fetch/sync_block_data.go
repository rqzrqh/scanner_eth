...
func (fm *FetchManager) syncNodeDataByHash(hash string) bool {
	...
	if header == nil {
		...
		header = fm.blockFetcher.FetchBlockHeaderByHash(nodeID, int(node.Height), client, hash)
		if header == nil {
			return false
		}
	...
	startTime := time.Now()
	fullBlock := fm.blockFetcher.FetchFullBlock(nodeID, int(height), client, header)
	costTime := time.Since(startTime)
	if fullBlock == nil {
		fm.nodeManager.UpdateNodeState(nodeID, costTime.Microseconds(), false)
		return false
	}

	blockData := &EventBlockData{
		StorageFullBlock:  ConvertStorageFullBlock(fullBlock, node.Irreversible),
		ProtocolFullBlock: ConvertProtocolFullBlock(fullBlock, node.Irreversible),
	}
	fm.setNodeBlockBody(hash, blockData)
	fm.nodeManager.UpdateNodeState(nodeID, costTime.Microseconds(), true)
	return true
}
